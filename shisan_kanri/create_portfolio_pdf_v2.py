...
# ── データ ──────────────────────────────────────────────────────
jp_tokutei = [
    ("ＪＡＣリク",       1956760,  985800),
    ("三井物産",         2013984, 1413936),
    ("三菱商事(特)",      684838,  339570),
    ("武田薬品",         1205600,  378840),
    ("蔵王産業",         1424645,  489918),
    ("ＮＴＴ(特)",      1257907,  242257),
    ("ＫＤＤＩ",         608016,  244224),
    ("オリックス",      1048307,  696280),
    ("三菱ＵＦＪ",       1347524,  569196),
    ("Ｅギャランティ(特)", 549600,  145200),
    ("三井住友",          262119,  209430),
    ("ジャックス",        376500,  108200),
    ("東京海上",          135468,  103032),
    ("第一ライフ",        115136,   82112),
...
jp_nisa_growth = [
    ("キヤノン",          446600,  -40700),
    ("三菱商事(NISA)",    444700,  184600),
    ("Ｅギャランティ(N)", 366400,  109200),
    ("ＮＴＴ(NISA)",     528850,  -66150),
]
jp_old_nisa = [("信越化学", 715700, 287400)]
...
jp_funds = [
    ("eMAXIS Slim\nオールカントリー\n（成長投資枠）", 4301834, 1189070, 3112764),
    ("eMAXIS Slim\nオールカントリー\n（つみたて枠）",  3400523, 1100483, 2300040),
]
jp_fund_total  = 4301834 + 3400523
jp_fund_profit = 1189070 + 1100483

us_tokutei = [
    ("VTI",    180, 211.55, 367.01, 10731078, 6492258),
    ("HDV",    750,  19.42,  28.25,  3441697, 1820947),
    ("SPYD",   100,  40.32,  49.39,   802291,  353591),
    ("AMBQ",    80,  34.98,  72.43,   941242,  525722),
]
us_nisa = [
    ("HDV(NISA)", 440, 21.86, 28.25, 2019129, 597049),
    ("PLTR",        7, 72.00,132.38,  150526,  73484),
]
...
us_bonds = [
    ("4.125% 2027/9満期",   12717.44, 2065820),
    ("4.750% 2053/11満期",  13178.10, 2140650),
    ("4.250% 2035/8満期",    3900.00,  633516),
    ("4.000% 2035/11満期",  14626.28, 2375892),
]
us_bond_total = 7215878

sbi_usd_cash   = 1_880_000
sbi_jpy_cash   = 3_500_000  # 2026/7 SKYT売却代金 約230万円を現金化して反映
...
def new_page(label='', title='', insight=''):
    ...
    f.text(0.04, 0.45, '個人投資ポートフォリオ  SBI証券  2026年7月19日',
...
def s1_cover():
    ...
    f.set_facecolor(FOOTER); f.axis('off')
    f.set_xlim(0,1); f.set_ylim(0,1)
    f.text(0.5, 0.45, '個人投資ポートフォリオ  SBI証券  2026年7月19日',
...
def s2_overview():
    ...
    blbls_labels = ['日本株\n特定','日本株\nNISA','日本株\n旧NISA',
                    '投信\n成長枠','投信\nつみたて',
                    '米国株\n特定','米国株\nNISA','米国株\n旧NISA','米国\n国債','SBI\n預り金']
    bvals = [jp_stock_total-1786550-715700, 1786550, 715700,
             4301834, 3400523,
...
def s4_jp_sector():
    sectors = {
        '金融':        1347524+262119+376500+1048307+30580+115136+135468+549600+366400,
        '商社':        2013984+684838+444700+199763,
        '通信':        1257907+528850+608016+29880,
        '化学・素材':  1205600+715700,
        '人材・サービス': 1956760+56544+41280+29104,
        'その他':      568431+100960+1424645+446600,
    }
...
def s5_jp_nisa():
    fig = new_page(
        ...
        insight='NISA成長投資枠 4銘柄（178.7万円 ｜ +18.7万）  ／  旧NISA 1銘柄（71.6万円 ｜ +28.7万）'
    )
    ...
    rows  = [
        ('キヤノン',        'NISA成長', '100株',   '4,873円', '4,466円', '446,600円', '▲40,700円'),
        ('三菱商事',        'NISA成長', '100株',   '2,601円', '4,447円', '444,700円', '+184,600円'),
        ('Eギャランティ',   'NISA成長', '200株',   '1,286円', '1,832円', '366,400円', '+109,200円'),
        ('NTT',             'NISA成長', '3,500株', '170円',   '151円',   '528,850円', '▲66,150円'),
        ('信越化学',        '旧NISA',   '100株',   '4,283円', '7,157円', '715,700円', '+287,400円'),
        ('合　計',          '',         '',         '',        '',        '2,502,250円','＋474,350円'),
    ]
    ...
    pts = [
        (TEAL,  '信越化学（旧NISA）+67.1%',
         '半導体材料需要増により大幅上昇。旧NISAで非課税保有。'),
        (BLUE1, '三菱商事（NISA）+71.0%',
         'NISA口座で+184,600円。資源価格上昇の恩恵を享受。'),
        (NEG,   'NTT・キヤノン 含み損',
...
def s7_us_stock():
    ...
    bclrs  = [AMBER]*4 + ['#E8A050']*2

    fig = new_page(
        label='米国株式 保有状況',
        title='米国株式 保有状況',
        insight=f"特定4 ＋ NISA2  ｜  円換算合計: {us_stock_total/1e4:.0f}万円  ／  評価益: +{us_stock_profit/1e4:.0f}万円"
    )
    gs = gs_body(fig, 1, 2, wspace=0.35)
...
def s8_us_bond():
    ...
    fig = new_page(
        label='米ドル建て国債 保有状況',
        title='米ドル建て国債 保有状況',
        insight=f"特定預り 4本  ｜  円換算合計: {us_bond_total/1e4:.0f}万円  ／  為替レート 約162円/USD（取得時157〜160円）"
    )
    gs = gs_body(fig, 1, 2, wspace=0.30)
...
def s9_performance():
    ...
    summary = [
        ...
        ('SBI預り金\n（USD+円）', f"{SBI_CASH_TOTAL/1e4:.0f}万円", '—', 'USD188万+円350万'),
...
def s10_features():
    ...
    note.text(0.5, 0.5,
              '注目銘柄  |  AMBQ +107%  ・  三井物産 +236%  ・  三菱UFJ +73%  ・  VTI +73%  ・  PLTR +84%  ・  信越化学 +67%',
